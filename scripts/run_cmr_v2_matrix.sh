...
MANIFESTS=(
  "${MANIFEST_DIR}/cmr_teacher_train_complete26_nonexternal.csv"
  "${MANIFEST_DIR}/cmr_teacher_val_complete26_nonexternal.csv"
  "${MANIFEST_DIR}/cmr_teacher_test_complete26_nonexternal.csv"
)
CLASSIFICATION="prevalent_I21,prevalent_I25,composite_cardiomyopathy_hf,prevalent_I48"
...
for FUSION_MODE in global_only sax_t1 hierarchical; do
  "${PYTHON_BIN}" train_cmr_teacher.py \
    ...
    --seed "${SEED}" \
    --output-dir "${OUTPUT_ROOT}/cmr_26task_${FUSION_MODE}_seed${SEED}"
done
