"""
train_cmr_v3_hpo.py  —  CMR v3 HPO sweep (composite_ischemic_hd only)

Differences from train_cmr_v3.py:
  - Only composite_ischemic_hd classification (I21 dropped)
  - --transformer_depth CLI arg
  - --unfreeze_batch_size tunable (try 6/8/10)
  - Default epochs=40, patience=8 (faster HPO cycle)
  - Regression tasks (LVEF/LVEDV/LVESV) kept as auxiliary supervision

Usage (4 GPUs in parallel on 2572):
  bash contrastive_pretrain/run_cmr_v3_hpo.sh
"""
...
# All available classification tasks and their pos_weights (neg/pos ratio)
ALL_CLS_TASKS = {
    'composite_ischemic_hd':    2.64,
    'prevalent_I21':            8.82,
    'prevalent_I20':            4.72,   # angina
    'prevalent_I22':           47.24,   # subsequent MI (very rare)
    'prevalent_I24':           24.80,   # other acute ischaemic
    'prevalent_I25':            3.83,   # chronic IHD
    'composite_cardiomyopathy_hf': 22.53,
    'composite_af_arrhythmia':  2.85,
}

# defaults (overridden by --cls_tasks at runtime)
CLS_COLS    = ['composite_ischemic_hd']
...
class CMRDatasetV3(Dataset):
    def __init__(self, csv_path: str, data_dir: str | None, is_train: bool = True):
        ...
        print(f'[dataset] {len(self.rows)} samples ({"train" if is_train else "val"})')
    ...
    def __getitem__(self, idx):
        ...
        assert npy.shape[0] == NUM_FRAMES, f'expected {NUM_FRAMES} frames, got {npy.shape[0]}'
...
def task_loss(cls_preds, reg_preds, cls_labels, reg_labels, reg_mask, device):
    cls_losses = []
...
def main():
    ...
    parser.add_argument('--cls_tasks',           type=str,
                        default='composite_ischemic_hd',
                        help='Comma-separated cls tasks, e.g. '
                             '"composite_ischemic_hd,prevalent_I20"')
    ...
    # ── resolve cls tasks ──
    global CLS_COLS, POS_WEIGHTS
    CLS_COLS    = [t.strip() for t in args.cls_tasks.split(',')]
    POS_WEIGHTS = [ALL_CLS_TASKS[t] for t in CLS_COLS]
    print(f'[tasks] cls={CLS_COLS}  pos_weights={POS_WEIGHTS}')
    ...
    if args.freeze_epochs > 0:
        ...
        print(f'[freeze] backbone frozen for {args.freeze_epochs} epochs')
    ...
    print(f'Params: {n_total/1e6:.1f}M  |  depth={args.transformer_depth}  '
          f'pool={args.spatial_pool}  bs={args.batch_size}/{args.unfreeze_batch_size}  '
          f'enc_lr={args.enc_lr:.1e}  head_lr={args.head_lr:.1e}')

    for epoch in range(start_ep, args.epochs):
        if args.freeze_epochs > 0 and epoch == args.freeze_epochs:
            for p in encoder.backbone.parameters():
                p.requires_grad_(True)
            encoder.grad_checkpoint = True
            train_loader, val_loader = make_loaders(args.unfreeze_batch_size)
            print(f'[unfreeze] epoch {epoch}  grad_checkpoint=True  '
                  f'bs → {args.unfreeze_batch_size}')
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = task_loss(cls_out, reg_out, cls_gt, reg_gt, reg_mk, device)
        ...
        for name in CLS_COLS:
            print(f'  {name}: {metrics.get(f"auc/{name}", float("nan")):.4f}')
    ...
    print(f'\nDone. best_auc={best_auc:.4f}  out_dir={out_dir}')
