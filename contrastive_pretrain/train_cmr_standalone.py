"""
train_cmr_standalone.py  —  P0.1 CMR baseline (single GPU)

CMREncoder (MedSAM ViT-B, full FT) + TaskHead on task1_cmr cohort.
Predicts: composite_ischemic_hd, prevalent_I21 (vascular diseases only)
          + LV ejection fraction / end diastolic volume / end systolic volume

Usage:
  CUDA_VISIBLE_DEVICES=0 python contrastive_pretrain/train_cmr_standalone.py \
      --out_dir contrastive_pretrain/checkpoints_cmr_standalone
"""
...
class CMRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CMR: (4, 224, 224) float16 → (4, 1, 224, 224) float32
        npy = np.load(r['path']).astype(np.float32)   # (4, 224, 224)
        if npy.ndim == 3:
            npy = npy[:, np.newaxis, :, :]             # (4, 1, 224, 224)
...
def task_loss(cls_preds, reg_preds, cls_labels, reg_labels, reg_mask,
    ...
    for i, pred in enumerate(cls_preds):
        ...
        loss = nn.BCEWithLogitsLoss(pos_weight=pw)(pred[valid], cls_labels[valid, i])
...
def evaluate(encoder, head, loader, device):
    ...
    for i, name in enumerate(REG_COLS):
        ...
        pred_denorm = pred[mask] * std + mu
        metrics[f'mae/{name}'] = float(np.abs(pred_denorm - gt[mask]).mean())
...
def main():
    ...
    parser.add_argument('--out_dir',    type=str,
                        default=str(HERE / 'checkpoints_cmr_standalone'))
    ...
    for epoch in range(start_ep, args.epochs):
        ...
        for name in REG_COLS:
            print(f'  {name}: MAE={metrics.get(f"mae/{name}", float("nan")):.2f}')
