"""trainval_tier2.py -- Stage 2 Tier 2 training (~5K EIDs).

Differences vs trainval_tier1.py:
  - Default CSV = fundus_table_extended.csv (covers same_inst + fund3->cmr2)
  - Train split is restricted to the 5K EIDs listed in `--train_eids_file`
    (default: contrastive_pretrain/stage2_tier2_5k_eids.txt)
  - Val and test use the full extended val/test splits (N~1.4K each)

Run two times: once unfrozen, once with --freeze_backbones for parallel compare.
"""
...
def eval_retrieval(model, dataset, device, batch_size: int = 16, eval_num_workers: int = 3,
    ...
    If max_gallery > 0 and N > max_gallery, subsample gallery to max_gallery (seeded).
    """
    model.eval()
    seen = set()
    idx_list = []
...
def main():
    ...
    ap.add_argument('--train_eids_file', type=str,
                    default='contrastive_pretrain/stage2_tier2_5k_eids.txt',
                    help='Restrict train split to these EIDs. Empty = use all train EIDs.')
    ...
    ap.add_argument('--train_augment', action='store_true',
                    help='Disabled by default -- caused collapse in Tier 1.')
    ap.add_argument('--val_max_gallery', type=int, default=0,
                    help='If >0, subsample val gallery to this many EIDs each eval '
                         '(for speed & comparison with Tier 1 scale). 0 = full gallery.')
    ...
    print(f'[tier2] device={device}  out_dir={args.out_dir}  freeze={args.freeze_backbones}')

    train_eids = None
    if args.train_eids_file and os.path.isfile(args.train_eids_file):
        train_eids = read_eids(args.train_eids_file)
        print(f'[tier2] train_eids_file = {args.train_eids_file}  count={len(train_eids)}')
    else:
        print('[tier2] no train_eids_file, using full train split')
    ...
    print(f'[tier2] train: {len(ds_train)} pairs, {len(set(ds_train.eids))} unique EIDs')
    print(f'[tier2] val  : {len(ds_val)} pairs, {len(set(ds_val.eids))} unique EIDs')
    print(f'[tier2] test : {len(ds_test)} pairs, {len(set(ds_test.eids))} unique EIDs')
    ...
    print(f'[tier2] iters/epoch = {n_iters_per_epoch}')
    ...
    print(f'[tier2] params: total={n_tot/1e6:.1f}M  trainable={n_tr/1e6:.2f}M')
    ...
    print(f'[tier2] base_lr={args.base_lr} temp={args.temperature} bs={args.batch_size} '
          f'ep={args.epochs} amp={args.amp} sched={args.lr_schedule} warmup={args.warmup_epochs}')

    m_val0 = eval_retrieval(model, ds_val, device, max_gallery=args.val_max_gallery)
    print('[tier2] epoch 0 [baseline]')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f'[tier2] ep {epoch:3d}/{args.epochs} loss={mean_loss:.4f} '
        ...
        if epoch % args.eval_every == 0 or epoch == args.epochs:
            m_val = eval_retrieval(model, ds_val, device, max_gallery=args.val_max_gallery)
            print('        ' + _fmt(m_val, 'val'), flush=True)
            log_entry = {'epoch': epoch, 'phase': 'eval', 'val_metrics': m_val}

            if args.train_eval_every and epoch % args.train_eval_every == 0:
                m_tr = eval_retrieval(model, ds_train_eval, device, max_gallery=500)
                print('        ' + _fmt(m_tr, 'train(subset=500)'), flush=True)
            ...
            if v_r1 > best_val_r1:
                ...
                save_encoder_checkpoint(os.path.join(args.out_dir, 'checkpoint_best.pth'),
                                        model, args, m_val)
                print(f'        [best] val R@1_avg={v_r1:.3f} saved at ep{epoch}', flush=True)

            with open(os.path.join(args.out_dir, 'history.json'), 'w') as f:
                json.dump(history, f, indent=2)

    print(f'\n[tier2] final test eval (best checkpoint from ep{best_epoch})', flush=True)
    ...
    m_test = eval_retrieval(model, ds_test, device, max_gallery=args.test_max_gallery)
    m_val_best = eval_retrieval(model, ds_val, device, max_gallery=args.val_max_gallery)
    print('        ' + _fmt(m_val_best, 'val(best)'), flush=True)
    print('        ' + _fmt(m_test, 'test'), flush=True)
    ...
    with open(os.path.join(args.out_dir, 'history.json'), 'w') as f:
        json.dump(history, f, indent=2)
    print(f'[tier2] done. total {(time.time()-t_start)/60:.1f} min')
    print(f'[tier2] best val R@1={best_val_r1:.3f} at ep{best_epoch}')
    print(f'[tier2] test R@1={m_test["R@1_avg"]:.3f} '
