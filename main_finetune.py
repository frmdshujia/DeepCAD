...
# 兼容新版 PyTorch：torch._six 已移除，timm/util 依赖它
import sys
...
def get_args_parser():
    ...
    # to describ a training(shujia adds)
    parser.add_argument('--desc', default='', type=str,
                        help='to describ a training')

    # which GPU
    parser.add_argument('--gpu', default='0', type=str, help='Specify GPU id to use')

    # FocalLoss 正类权重：alpha 越大正类权重越高，建议 0.8~0.95 应对阳性稀少
    parser.add_argument('--focal_alpha', default=0.25, type=float,
                        help='FocalLoss alpha for positive class (default 0.25, use 0.9 for imbalanced)')
    parser.add_argument('--focal_gamma', default=1.5, type=float,
...
def main(args):
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu #通过引入参数告知使用哪张GPU
    ...
    dataset_train = MyCustomData(is_train='train', args=args)
    dataset_val = MyCustomData(is_train='val', args=args)
    dataset_test = MyCustomData(is_train='test', args=args)
    ...
    if args.finetune and args.finetune.lower() != 'none' and not args.eval:
    ...
    if args.eval:
        test_stats,auc_roc = evaluate(data_loader_test, model, device, args, epoch=0, mode='test',num_class=args.nb_classes)
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_stats,val_auc_roc = evaluate(data_loader_val, model, device,args,epoch, mode='val',num_class=args.nb_classes)
        ...
        if args.output_dir:
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, suffix=epoch)

        if epoch==(args.epochs-1):
            test_stats,auc_roc = evaluate(data_loader_test, model, device,args,epoch, mode='test',num_class=args.nb_classes)
